...
OUTPUT_PATH = os.path.join(EDITOR_DIR, "(patched)save_coop_0.sav")


class SaveEditorApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Dying Light Save Editor")
    ...
    def populate(self):
        ...
        self.create_table(self.tab_weapons, ["name","quantity","condition","repairs","id"], weapons, self.weapon_map)

        self.create_table(self.tab_inventory, ["name","quantity","id"], items, self.inventory_map)
    ...
    def create_stats(self, parent, player):
        for key in ["health", "fury"]:
            ...
            var = tk.StringVar(value=str(player.get(key, 0)))
